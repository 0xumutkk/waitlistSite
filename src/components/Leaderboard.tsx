...
function SkeletonRow({ isLast }: { isLast?: boolean }) {
  return (
    <div className={`flex items-center w-full border-b border-black/15 ${isLast ? 'border-b-0' : ''} animate-pulse`}>
      ...
      <div className="w-[157px] h-[56px] flex items-center px-[8px] border-r border-black/15">
        <div className="flex items-center gap-[4px] pl-2 w-full justify-center">
          <div className="w-[20px] h-[20px] rounded-full bg-black/10 flex-shrink-0" />
        ...
        <div className="flex flex-col w-full">
          {showSkeleton ? (
            SKELETON_ROWS.map((_, i) => (
              <SkeletonRow key={i} isLast={i === SKELETON_ROWS.length - 1} />
            ))
          ) : (
            entries.map((entry, i) => {
